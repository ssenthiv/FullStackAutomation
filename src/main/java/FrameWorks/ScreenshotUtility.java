...
public class ScreenshotUtility extends Base implements ITestListener {
	...
	public void onTestSuccess(ITestResult tr) {
		...
		//if (Base.Param.getProperty("screenShotOnPass").equalsIgnoreCase("yes")) {
			
				 object =tr.getTestClass().getInstances(true);
			System.out.println("size : "+object.length);
			driver=((Base) object[0]).driver;
			/*
			for (Object ele: object){
				driver=((Base) ele).driver;
				System.out.println("Object =" +ele.toString());
			}
			*/
	...
	public void onTestFailure(ITestResult tr) {
		...
		//if (Base.Param.getProperty("screenShotOnFail").equalsIgnoreCase("yes")) {
